...
/* accessor method for mode data field
 @return - the current mode the app is it (Daily/Reoccurring).
 */
int getMode(void){
    return mode;
}
...
/*setMessage method determines the correct phrase for each reoccurring habit*/
void setMessage(void){
    ...
    if((getSelectedIntervalHours() == 0 || getSelectedIntervalHours() == 1) && getSelectedIntervalMinutes() != 0 && getSelectedIntervalMinutes() != 1){
        
        message = "Every " + std::to_string(getSelectedIntervalHours()) + " hour and "
            + std::to_string(getSelectedIntervalMinutes()) + " minutes";
        charConversion = new char[message.length()+1];
        strcpy(charConversion, message.c_str());
        setAlertOptionsMode1Text(charConversion);
        printf("%s",charConversion);
        delete [] charConversion;
    }
    
    else if((getSelectedIntervalHours() == 0 || getSelectedIntervalHours() == 1) && (getSelectedIntervalMinutes() == 0 || getSelectedIntervalMinutes() == 1)){

        message = "Every " + std::to_string(getSelectedIntervalHours()) + " hour and "
        + std::to_string(getSelectedIntervalMinutes()) + " minute";
        ...
        message = "Every " + std::to_string(getSelectedIntervalHours()) + " hours and "
        + std::to_string(getSelectedIntervalMinutes()) + " minute";
        charConversion = new char[message.length()+1];
        strcpy(charConversion, message.c_str());
        setAlertOptionsMode1Text(charConversion);
    ...
    if(getSelectedIntervalHours() == 0 && getSelectedIntervalMinutes() != 0 && getSelectedIntervalMinutes() != 1){
        
        message = "Every " + std::to_string(getSelectedIntervalMinutes())+ " minutes";
        charConversion = new char[message.length()+1];
        strcpy(charConversion, message.c_str());
        setAlertOptionsMode1Text(charConversion);
        printf("%s",charConversion);
    ...
    }else if(getSelectedIntervalMinutes() == 0 && getSelectedIntervalHours() != 0 && getSelectedIntervalHours() != 1){
        
        message = "Every " + std::to_string(getSelectedIntervalHours()) + " hours";
        charConversion = new char[message.length()+1];
        strcpy(charConversion, message.c_str());
        setAlertOptionsMode1Text(charConversion);
        printf("%s",charConversion);
    ...
    }else if(getSelectedIntervalHours() == 0 && (getSelectedIntervalMinutes() == 0 || getSelectedIntervalMinutes() == 1)){
        
        message = "Every " + std::to_string(getSelectedIntervalMinutes())+ " minute";
        charConversion = new char[message.length()+1];
        strcpy(charConversion, message.c_str());
        setAlertOptionsMode1Text(charConversion);
        printf("%s",charConversion);
        ...
        message = "Every " + std::to_string(getSelectedIntervalHours())+ " hour";
        charConversion = new char[message.length()+1];
        strcpy(charConversion, message.c_str());
        setAlertOptionsMode1Text(charConversion);
        printf("%s",charConversion);
